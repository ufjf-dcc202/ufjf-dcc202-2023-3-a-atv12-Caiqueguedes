...
test.describe("estrutura do HTML", () => {
  ...
  test("o form deve dois fieldset com legendas Origem e Destino", async ({
    page,
  }) => {
    await page.goto(`http://localhost:5500`);
    const f = await page.locator("main > section > form");
  ...
  test("O primeiro fieldset deve ter três radios com valores para cada personagem'", async ({
    page,
  }) => {
    await page.goto(`http://localhost:5500`);
    const radio1 = page.locator(
      "fieldset:nth-child(1) > label:nth-child(2) > input[type=radio]"
    );
    const radio2 = page.locator(
      "fieldset:nth-child(1) > label:nth-child(3) > input[type=radio]"
    );
    const radio3 = page.locator(
      "fieldset:nth-child(1) > label:nth-child(4) > input[type=radio]"
    );
  ...
  test("O segundo fieldset deve ter três radios com valores para cada personagem'", async ({
    page,
  }) => {
    await page.goto(`http://localhost:5500`);
    const radio1 = page.locator(
      "fieldset:nth-child(2) > label:nth-child(2) > input[type=radio]"
    );
    const radio2 = page.locator(
      "fieldset:nth-child(2) > label:nth-child(3) > input[type=radio]"
    );
    const radio3 = page.locator(
      "fieldset:nth-child(2) > label:nth-child(4) > input[type=radio]"
    );
  ...
  test("Deve ter um campo para entrada de quantidades em número e com limites", async ({
    page,
  }) => {
    await page.goto(`http://localhost:5500`);
...
test.describe("comportamento", () => {
  test("João deve ser capaz de doar suas maçãs para maria", async ({
    page,
  }) => {
    await page.goto("http://localhost:5500/");
    ...
    await page.getByRole("button", { name: "Enviar" }).click();
    await page
      .getByRole("group", { name: "Origem" })
      .getByLabel("João")
      .check();
    await page
      .getByRole("group", { name: "Destino" })
      .getByLabel("Maria")
      .check();
  ...
  test("Maria deve ser capaz de doar suas maçãs e peras para joão", async ({
    page,
  }) => {
    await page.goto("http://localhost:5500/");
    await page.getByRole("button", { name: "Limpar" }).click();
  ...
  test("A lista deve ter o mesmo conteúdo inicial do estado com 1 e 2 maçãs para joão e maria", async ({
    page,
  }) => {
    await page.goto(`http://localhost:5500`);
    const olJoao = await page.locator("main > section#saida > ol#joao");
  ...
  test("A lista deve ficar vazia ao clicar em 'Limpar Lista'", async ({
    ...
    await page.goto(`http://localhost:5500`);
    const liDeOlJoao = await page.locator(
      "main > section#saida > ol#joao > li"
    );
